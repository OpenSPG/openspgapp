...
public class SchemaPermissionServiceImpl implements SchemaPermissionService {
    ...
    @Autowired
    private SchemaPermissionRepository schemaPermissionRepository;
    ...
    @Override
    public Boolean verifyRole(String userId, Long resourceId, ResourceTypeEnum resourceTypeEnum, RoleTypeEnum roleTypeEnum) {
        ...
        List<SchemaPermission> schemaPermissionList = schemaPermissionRepository.queryByCondition(schemaPermission);
    ...
    @Override
    public Integer addResourceRole(String token, List<String> userIds, List<Long> resourceIds, ResourceTypeEnum resourceTypeEnum,
                                   RoleTypeEnum roleTypeEnum) throws Exception {
    ...
    @Override
    public Integer removeRole(String token, List<Long> userIds, List<Long> resourceIds, ResourceTypeEnum resourceTypeEnum) throws Exception {
        checkOwner(token, resourceIds, resourceTypeEnum);
        return schemaPermissionRepository.batchRemove(userIds, resourceIds, resourceTypeEnum);
    }

    private void checkOwner(String token, List<Long> resourceIds, ResourceTypeEnum resourceTypeEnum) throws Exception {
        User user = accountRepository.queryByToken(token);
        if (null == user) {
            throw new Exception("invalid user");
        }

        List<SchemaPermission> schemaPermissionList = schemaPermissionRepository.queryByResourceIds(resourceIds, resourceTypeEnum);
